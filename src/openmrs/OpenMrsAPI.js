...
class OpenMrsAPI {
  ...
  createPatient(data, locationUUID) {
      ...
      let postalCode = ""
      this.createPerson(data['family'], data['given'], data['sex'], data['dob'], data['address'], data['location'], data['country'], postalCode).then(person =>{
        console.log("***********Created person**************")
        console.log(person.body)
        let person_body = person.body

        this.getLocation(data).then(location=>{
          console.log("****************Location created************")
          this.generateOpenMRSID().then(openMRSIDResult=>{
      ...
      if (data["next_facility_to_visit_transfered"]){
        ...
        if (data["anc_first_hiv_test_status"]) {
          obs.push({
              ...
              "value": uuids.odkYesNo[data["anc_first_hiv_test_status"]]
            })
          }
